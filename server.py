...
API_VERSION = "2.5.0" # Added Tap-to-Target feature
...
# НОВЫЙ МЕТОД: Отрисовка ВСЕХ деревьев в лесу
def draw_all_masks(img_bgr, masks_list, selected_idx):
    for i, mask in enumerate(masks_list):
        cnts, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if i == selected_idx:
            color = (0, 255, 0) # Зеленый контур для целевого дерева (BGR)
            thickness = 3
        else:
            color = (255, 0, 0) # Синий контур для остальных деревьев (BGR)
            thickness = 2
            
        for cnt in cnts:
            approx = cv2.approxPolyDP(cnt, 0.003 * cv2.arcLength(cnt, True), True)
            cv2.drawContours(img_bgr, [approx], -1, color, thickness)
            
            # Пишем номер над деревом (для наглядности)
            if len(approx) > 0:
                x, y, w, h = cv2.boundingRect(approx)
                text_color = (0, 255, 0) if i == selected_idx else (255, 150, 150)
                cv2.putText(img_bgr, f"#{i+1}", (x, max(20, y - 10)), cv2.FONT_HERSHEY_SIMPLEX, 1.0, text_color, 2)
...
app = FastAPI(title="ArborScan API v3.0 (Forest Edition)")
...
def _run_yolo_sync(img_array):
    return get_tree_model()(img_array, imgsz=1024, retina_masks=True, conf=0.10)[0], stick_model(img_array)[0]
...
async def analyze_tree(
    ...
    tap_x: Optional[float] = Form(None), # <--- НОВАЯ ПЕРЕМЕННАЯ (СНАЙПЕРСКИЙ ТАП ПО ИКСУ)
    tap_y: Optional[float] = Form(None), # <--- НОВАЯ ПЕРЕМЕННАЯ (СНАЙПЕРСКИЙ ТАП ПО ИГРЕКУ)
    ...
    if tree_res.masks is None or len(tree_res.masks.data) == 0:
        ...
        for i, m in enumerate(tree_res.masks.data):
            ...
            # 2. ИНТЕРАКТИВНЫЙ ВЫБОР ДЕРЕВА (СНАЙПЕРСКИЙ ТАП)
            if tap_x is not None and tap_y is not None:
                ...
                if tmp_mask[check_y, check_x] > 0:
                    idx = i # Идеальное попадание!
                    
            x1, y1, x2, y2 = tree_res.boxes.xyxy[i].cpu().numpy()
            dist = (((x1+x2)/2.0) - target_x)**2 + (((y1+y2)/2.0) - target_y)**2
            distances.append(dist)

        # Если юзер промахнулся мимо маски или не тапал вообще — берем ближайшее дерево
        if idx == -1 and distances:
            idx = int(np.argmin(distances))

    yolo_mask = masks[idx]

    # 3. ИДЕАЛЬНАЯ МАСКА (REMBG + OPENCV SOLID FILL) ТОЛЬКО ДЛЯ ВЫБРАННОГО ДЕРЕВА
    try:
    ...
    # --- ОТРИСОВКА ВСЕХ ДЕРЕВЬЕВ В КАДРЕ ---
    annotated_b64 = draw_all_masks(img.copy(), masks, idx)
...
def send_feedback(payload: dict = Body(...)):
    # Функция обратной связи (убрана для краткости)
    return {"status": "ok"}